...
try:
    ...
    with col3:
        ...
        if 2 <= Nationality <= 21:
            ...
            if prediction[0] == 1:
                st.markdown(f"<h2 style='font-size: 30px; color: green;'>Dropout！</h2>", unsafe_allow_html=True)
            elif prediction[0] == 0:
                st.markdown(f"<h2 style='font-size: 30px; color: green;'>Graduate！</h2>", unsafe_allow_html=True)
